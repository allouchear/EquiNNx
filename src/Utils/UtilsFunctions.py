...
def getArguments():
	...
	parser.add_argument("--data_loading", type=str, default='disk', choices=['ram','disk'],  help="only for a pre-built batching dataset : ram => load all batches in memory at startup ; disk => read batches on demand from the batching file (default, low RAM, disk I/O per batch per epoch)")
	parser.add_argument("--data_cache", type=int, default=1,  help="only for a pre-built batching dataset with --data_loading=disk : number of batches kept in an LRU cache (-1 = keep all batches after the first epoch). Default=1")

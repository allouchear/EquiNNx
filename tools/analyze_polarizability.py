...
if len(sys.argv)<2:
        print("Usage :")
        print("      give the name of npz file , idx alpha (xx, xy, yy,...) and the prefix name of the output file")
        print("      Example ", sys.argv[0], " train_dir/metrics/best_valid_valid_set.npz ", " xx ", " alfa ")
        exit(1)
...
predict=values['polarizability_predict'][:,i,j].reshape(-1)
target=values['polarizability_target'][:,i,j].reshape(-1)

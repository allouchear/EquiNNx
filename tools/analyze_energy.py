...
if len(sys.argv)<2:
        ...
        print("      Example ", sys.argv[0], " train_dir/metrics/best_valid_valid_set.npz ", " 20 ", " alfa ")
        exit(1)

data_file   = np.load(sys.argv[1],allow_pickle=True)
values=data_file['values'].item()
predict=values['energy_predict'].reshape(-1)
target=values['energy_target'].reshape(-1)
